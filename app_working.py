...
def encode_text_file_to_image(input_text, input_file_path):
    if input_text:
        text = input_text
    ...
    if input_file_path:
        ...
        # Encode text into a single image if possible
        encoded_image = encode_text_to_image(text)
        
        # Save encoded image to downloads folder
        output_image_path = os.path.join(os.path.expanduser('~'), 'Downloads', f"{os.path.splitext(os.path.basename(input_file_path))[0]}_encoded.png")
...
def encode():
    ...
    output_location = request.form.get('output_location')  # Get user-defined output location
    ...
    encoded_image_paths = encode_text_file_to_image(text_input, file_path)

    # Redirect to index page with result
    return redirect(url_for('index', result=f'Encoded image(s) saved at: {encoded_image_paths}'))
...
def decode():
    ...
    output_text_file_path = decode_image_file_to_text([file_path])

    # Redirect to index page with result
    return redirect(url_for('index', result=f'Decoded text saved at: {output_text_file_path}'))
